...
# Should reestimate this model; Not sure should be classified in the middle, not
# as missing. Need to adjust in the original data and reestimate the model
  
output$hist_separate <- renderPlotly({
  # create a histogram of the data
  # Create the base histogram
  if(input$surveyweight == "No Survey Weights"){
    df  =  build_data_agree(item = "separate_parents")[[2]]
  }
  else{
    df  =  build_data_agree(item = "separate_parents")[[3]]
  }
  
  model_df = build_data_agree(item = "separate_parents")[[1]]
 ...
 mutate(.category = factor(.category, 
  ...
  base =  plot_ly(df, x = ~.category, 
                  ...
                  hoverinfo =  "text",
                  opacity = 0.3) %>% 
    layout(title = "'End the separation of parents and children\nat the border'",
           showlegend = TRUE,
