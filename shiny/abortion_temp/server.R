...
server <- function(input, output) {
  ...
  create_plot_multi <- function(outcome_name, plot_title, recode_scheme = "oppose5r", show_legend = TRUE) {
    ...
    renderPlotly({
      ...
      base <- plot_ly(df, x = ~.category, 
                      ...
                      height = 500,
                      opacity = 0.3) %>% 
        layout(title = list(text = plot_title, automargin=TRUE),
...
output$hist_jail <- create_plot_multi(
    outcome_name = "abortion_jail",
    plot_title = "",
    recode_scheme = "oppose5r"
  )  
output$hist_legal <- create_plot_multi(
  outcome_name = "abortion_legal", 
  plot_title = "", 
  recode_scheme = "abortion")
